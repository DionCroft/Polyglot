...
- 92 tests pass on each Windows runtime, with two POSIX-only skips. Mandarin model and UI smoke tests pass on CPU, Qualcomm NPU and emulated x64/DirectML. Both rebuilt Windows executables pass English/Mandarin inference, switching and transcript checks; all bundled model hashes and portable ZIP contents are verified. Native macOS release checks are in progress; see the evidence report.
